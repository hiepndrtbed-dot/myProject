...
function newLayerMask() {
	...
	if (hasActiveDocument()) {
		//
		activeDocument.suspendHistory("V V V   NLM_plus script START   V V V",'');
		...
		if (hasActivePath_onYes_selectIt()) {
			...
			if (hasSelection()) {
				...
				return "FX NLM_plus (Err: Path didn't create any visible selection)";
			}
		} else {
			...
			if (hasSelection()) {
				...
				return "FX NLM_plus (Err: Path or selection not existing)";
			}
		}
	...
	function fillWhite() {
		var descriptor = new ActionDescriptor();
		descriptor.putEnumerated( stringIDToTypeID( "using" ), stringIDToTypeID( "fillContents" ), stringIDToTypeID( "white" ));
