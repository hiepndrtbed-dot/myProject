...
function ColorBackground(vr, color) {
    ...
    if (grItem.searchLayerInGroup("#Floor")[0] == true) {
        ...
        if (hasMask() && loadSelectionMask() == true) {
            ...
            // grItem.selectGroup()
            //Chọn Group item 1 để lấy mask Product.
            ...
            for (var i = 0; i < lengthItem; i++) {
                if (doc.activeLayer.layers[i].name.search("Product") == 0) {
                    ...
                    if (hasMask()) {
                        ...
                        grColor.selectGroup()
                        pasteFolder()
                        doc.activeLayer.name = "Floor_temp"
                        doc.activeLayer.visible = false
                        pasteFolder()
                        ...
                        try { replaceColorBackgroundPointWhite("Pr_WarBG", "BG_Wall_Curves", "BG_Wall_Desat_100", "BG_Wall_Blend_ece8e0", 236, 232, 224, 94) } catch (error) { }
                        try { doc.channels.getByName("Temp_SubtractSelection").remove(); } catch (err) { }
                        //floor
                        grColor.selectLayer("Floor_temp")
                        doc.activeLayer.visible = true
                        doc.activeLayer.move(doc.layerSets["Variant " + vr].layerSets["Color " + vr], ElementPlacement.PLACEBEFORE)
                        moveDown()
                        ...
                        doc.selection.deselect()
                        makeThreshold("light", 255)
                        try { replaceColorBackgroundPointPloor("Floor_colorBG", "BG_Floor_Curves", "BG_Floor_Desat_100", "BG_Floor_Blend_ece8e0", 247, 244, 241) } catch (error) { }
                        grColor.selectLayer("Floor_temp")
            ...
            //Top
            top()
            try { replaceColorBackgroundPointWhite("Pr_WarBG", "BG_Wall_Curves", "BG_Wall_Desat_100", "BG_Wall_Blend_ece8e0", 236, 232, 224) } catch (error) { }
            try { doc.channels.getByName("Temp_SubtractSelection").remove(); } catch (err) { }
            try { doc.channels.getByName("Pr_WarBG").remove(); } catch (err) { }
        }
    } else {//End Floor and wall 
        top()
        try { replaceColorBackgroundPointWhite("Pr_WarBG", "BG_Wall_Curves", "BG_Wall_Desat_100", "BG_Wall_Blend_ece8e0", 236, 232, 224) } catch (error) { }
        try { doc.channels.getByName("Temp_SubtractSelection").remove(); } catch (err) { }
        try { doc.channels.getByName("Pr_WarBG").remove(); } catch (err) { }
...
function replaceColorBackgroundPointWhite(channelSelection, nameCurves, nameHueColor, nameSolidColor, red, green, blue) {
    ...
    var pointDark = doc.colorSamplers[1]
    //Auto layer BG_Wall_Curves
    var rgbDark = {
        hue: Math.floor(pointDark.color.hsb.hue),
        brightness: pointDark.color.hsb.brightness,
        saturation: Math.floor(pointDark.color.hsb.saturation)
    }
    // alert(rgbDark.brightness)
    i = 2
    horizontalDark = 0
    verticalDark = 50
    horizontalLight = 255
    ...
    if (rgbDark.brightness > 90) {
        brightness = 93
    }
    else if (rgbDark.brightness > 86) {
        brightness = 91
    }
    else if (rgbDark.brightness > 79) {
        brightness = 87
    }
    else if (rgbDark.brightness > 75) {
        brightness = 85
    }
    else if (rgbDark.brightness > 73) {
        brightness = 83
    }
    else if (rgbDark.brightness > 70) {
        brightness = 81
    }
    else if (rgbDark.brightness > 67) {
        brightness = 79
    }
    else if (rgbDark.brightness < 67) {
        brightness = 75
    ...
    //xu ly de chay nhanh hon
    if (rgbDark.brightness < brightness) {
        grColor.selectLayer(nameCurves)
        selectRGB()
        setCurverHightLight(horizontalDark, verticalDark, horizontalLight, verticalLight)
        grColor.selectLayer(nameSolidColor)
        selectRGB()
        rgbDark.brightness = pointDark.color.hsb.brightness

        while (rgbDark.brightness < brightness) {
            grColor.selectLayer(nameCurves)
            selectRGB()
            verticalDark = verticalDark + i
            setCurverHightLight(horizontalDark, verticalDark, horizontalLight, verticalLight)
            grColor.selectLayer(nameSolidColor)
            selectRGB()
            rgbDark.brightness = pointDark.color.hsb.brightness
    ...
    j = 0
    while (rgbLight.brightness > 94.5) {
        grColor.selectLayer(nameCurves)
        selectRGB()
        setCurverHightLight(horizontalDark, verticalDark + j + 10, horizontalLight, verticalLight - j)
        grColor.selectLayer(nameSolidColor)
        selectRGB()
...
function replaceColorBackgroundPointPloor(channelSelection, nameCurves, nameHueColor, nameSolidColor, red, green, blue) {
    // doc.colorSamplers.remove()
    searchPointLight()
    doc.selection.deselect()
    ...
    var pointLight = doc.colorSamplers[0]
    //Auto layer BG_Wall_Curves
    var rgb = {
        hue: Math.floor(pointLight.color.hsb.hue),
        brightness: pointLight.color.hsb.brightness,
    ...
    alert(rgb.brightness)
    //kiem tra diem anh.
    if (rgb.brightness < brightness) {
        grColor.selectLayer(nameCurves)
        selectRGB()
        setCurverHightLight(horizontalDark, verticalDark, horizontalLight, verticalLight)
        grColor.selectLayer(nameSolidColor)
        selectRGB()
        rgb.brightness = pointLight.color.hsb.brightness

        while (rgb.brightness < brightness) {
            alert(1)
            grColor.selectLayer(nameCurves)
            selectRGB()
            setCurverHightLight(horizontalDark, verticalDark, horizontalLight - i, verticalLight)
            grColor.selectLayer(nameSolidColor)
            selectRGB()
            rgb.brightness = pointLight.color.hsb.brightness
            i = i + 1
        ...
        rgb.brightness = Math.floor(pointLight.color.hsb.brightness)
...
function searchPointLightDark() {
    ...
    //Tim diem toi
    doc.selection.load(doc.channels.getByName("Temp_SubtractSelection"))
    doc.selection.expand(3)
    fillColor(255, 255, 255)
    doc.selection.deselect()
    makeThreshold("Dark", 0)
    checkSelection = hasSelection()
    if (checkSelection == false) {
        for (var i = 0; i < 255; i = i + 5) {
            ...
            if (checkSelection == true) {
                ...
                j = 0
                //Kiem tra mau dung chua.
                while (maxNumberDark == 255) {
                    maxFor = j
                    //Point light
                    try { pointDark.remove() } catch (error) { }
                    bounds = doc.selection.bounds;
                    x_dark = bounds[0] + Math.floor(Math.random() * (bounds[2] - bounds[0]))
                    y_dark = bounds[1] + Math.floor(Math.random() * (bounds[3] - bounds[1]))
                    x_dark < 4 ? x_dark = x_dark + 5 : ""
                    y_dark < 4 ? y_dark = y_dark + 5 : ""
                    x_dark > doc.width - 4 ? x_dark = doc.width - 5 : ""
                    y_dark > doc.height - 4 ? y_dark = doc.height - 5 : ""

                    var pointDark = doc.colorSamplers.add([Math.floor(x_dark), Math.floor(y_dark)])
                    ...
                    //Tìm số lớn nhất R - G - B
                    bigNumber = 0
                    for (var i = 0; i < rgbDark.length; i++) {
                        if (bigNumber < rgbDark[i]) {
                            bigNumber = rgbDark[i]
                        }
                    }
                    maxNumberDark = bigNumber
                    if (maxFor == 15) {
                        pointDark.remove()
                        var pointDark = doc.colorSamplers.add([bounds[0] + 5, Math.floor(bounds[3] - 15)])
                        break
                    }
    ...
    if (checkSelection == false) {
        for (var i = 0; i < 128; i = i + 1) {
            ...
            if (checkSelection == true) {
                ...
                j = 0
                //Kiem tra mau dung chua.
                while (level > maxNumber - 2 && maxNumber == 0) {
                    maxLight = j
                    //Point light
                    try { pointTop.remove() } catch (error) { }
                    bounds = doc.selection.bounds;
                    ...
                    y > doc.heigth - 4 ? y = doc.heigth - 5 : ""

                    var pointTop = doc.colorSamplers.add([Math.floor(x), Math.floor(y)])
                    ...
                    if (maxLight == 15) {
                        pointTop.remove()
                        var pointTop = doc.colorSamplers.add([bounds[2] - 5, Math.floor(bounds[3] - 15)])
                        break
                    }
                    j = j + 1
