...
//version 1
savePathMask(1, namPath, nameChannel)

function savePathMask(vr, namePath, nameChannel) {
    ...
    for (var i = 0; i < doc.activeLayer.layers.length; i++) {
        ...
        if (nameLayer.search("Product") == 0) {
            ...
            if (hasVectorMask() || hasMask()) {
                ...
                //Kiểm tra tồn tại Mask và Flag == false khi chưa được lưu mask
                //Save vung chon tu mask
                if (hasMask() && FlagMask == false) {
                    loadSelectionMask()
                    saveSelection(nameChannel)
