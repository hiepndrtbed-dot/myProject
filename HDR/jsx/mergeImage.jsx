...
if (doc.layers.length == 4 || doc.layers.length == 5) {
    if (doc.layers.length == 5) {
        doc.artLayers[doc.layers.length - 5].remove();
    }
    doc.activeLayer = doc.artLayers[doc.layers.length - 4];
    loadSelectionChannel();
    addMask();
    doc.activeLayer.applyGaussianBlur(gaussianBlur);
    makeGroup("HighDarken");
    addMaskBlack();
}
var layerDarken = doc.layers[doc.layers.length - 3]
