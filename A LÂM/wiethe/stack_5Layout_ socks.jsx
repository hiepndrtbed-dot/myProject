...
(function () {
    //width body product
    const widthPr = widthLayout - (margin * 2 + padding * 4)
    const heightPr = heightLayout - margin * 2
    ...
    for (var i = 0; i < lengthVr; i++) {
        ...
        Algn("ADSCentersV") //"ADSCentersV" Doc ADSLefts
        Algn("ADSLefts") //"ADSCentersV" Ngang
        doc.selection.deselect()

        //Sắp xếp layer
        if (i == 0) {
            //Translate layer đầu
            doc.activeLayer.translate(margin, 0)
        } else {
            doc.activeLayer.translate(margin + padding * i, 0)
        }
    }
})()
