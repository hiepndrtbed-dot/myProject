...
const myPaths = app.activeDocument.pathItems
const padding = 50
const margin = 150
main()
function main() {
...
function transform() {
    ...
    doc.activeLayer = doc.layers["Layout"]
    var bounds = doc.activeLayer.bounds;
    crop(bounds[1], bounds[0], bounds[3], bounds[2]);

    var width = doc.width - margin * 2
    var height = doc.height - margin * 2

    var grBust = doc.layerSets["BUST"]
    doc.activeLayer = grBust
    doc.activeLayer = grBust.artLayers[0]

    var nameTop = doc.activeLayer.name
    ...
    loadSelectionMask()
    action("CpTL")
    doc.activeLayer.name = "Tem"
    grBust.artLayers["Tem"].link(grBust.artLayers[nameTop])
    doc.activeLayer = grBust.artLayers[nameTop]
    doc.activeLayer.visible = false
    selectGroupLayer("Tem")
    makeGroup()

    doc.selection.selectAll()
    Algn("ADSCentersH") //"ADSCentersV" Doc
    Algn("ADSBottoms") //"ADSBottoms" bottom

    unGroup()
    doc.activeLayer = grBust.artLayers[nameTop]
    doc.activeLayer.visible = true

    doc.activeLayer = grBust.artLayers["Tem"]
    grBust.artLayers['Tem'].remove()

    doc.selection.deselect()
    doc.activeLayer = grBust.artLayers[1]
    var nameTop = doc.activeLayer.name
    resizeImage(width, height)
    // 
    loadSelectionMask()
    ...
    doc.selection.deselect()
    loadSelectionMask()
    //Tính chiều cao của quần.
    var bounds1 = doc.selection.bounds
    var widthS = doc.height - bounds1[1]

    doc.selection.deselect()
    doc.activeLayer = grBust.artLayers[0]
    doc.activeLayer.translate(0, -widthS)


    doc.activeLayer = grBust
    resizeImage(width, height - padding)
    show()
    merge()
    doc.activeLayer.move(grBust, ElementPlacement.INSIDE)
    moveDown()
    doc.activeLayer = doc.layers["Layout"]
    doc.activeLayer = doc.layers["Farbfüllung 1"]
    doc.activeLayer = grBust.artLayers[1]
    doc.activeLayer.visible= false
    doc.activeLayer = grBust.artLayers[2]
    doc.activeLayer.visible= false
    ...
    doc.activeLayer.translate(0, -(margin + padding))
    doc.activeLayer = grBust.artLayers[1]
    doc.activeLayer.translate(0, padding)
    ...
    doc.selection.deselect()
    selectVectormask()
    savePathOfVectorpath("path_Temp")
    deletePathItem()
    mergePathItems()
    doc.activeLayer = grBust
...
function mergePathItems() {
    ...
    doc.activeLayer = doc.layerSets["BUST"].artLayers[namePathEnt]
    selectGroupLayer(namePathStart)
    uniteShape()
    combinePathItem()
...
// Algn("ADSCentersH"); //"ADSCentersV" ;"ADSBottoms"
...
function show() {
    ...
    var idnull = charIDToTypeID("null");
    var list142 = new ActionList();
    var ref779 = new ActionReference();
    var idLyr = charIDToTypeID("Lyr ");
    var idOrdn = charIDToTypeID("Ordn");
    var idTrgt = charIDToTypeID("Trgt");
    ref779.putEnumerated(idLyr, idOrdn, idTrgt);
    list142.putReference(ref779);
    desc1191.putList(idnull, list142);
    var idTglO = charIDToTypeID("TglO");
    desc1191.putBoolean(idTglO, true);
    executeAction(idShw, desc1191, DialogModes.NO);
...
function unShow() {
    ...
    var idnull = charIDToTypeID("null");
    var list143 = new ActionList();
    var ref780 = new ActionReference();
    var idLyr = charIDToTypeID("Lyr ");
    var idOrdn = charIDToTypeID("Ordn");
    var idTrgt = charIDToTypeID("Trgt");
    ref780.putEnumerated(idLyr, idOrdn, idTrgt);
    list143.putReference(ref780);
    desc1192.putList(idnull, list143);
    var idTglO = charIDToTypeID("TglO");
    desc1192.putBoolean(idTglO, true);
    executeAction(idShw, desc1192, DialogModes.NO);
...
function merge() {
    ...
    desc1195.putBoolean( idDplc, true );
executeAction( idMrgV, desc1195, DialogModes.NO );
}
