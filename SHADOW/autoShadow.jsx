...
(function () {
    ...
    function autoShadow(vr) {
        grVariant = doc.layerSets["Variant " + vr]
        grColor = grVariant.layerSets["Color " + vr]
        grItem = grVariant.layerSets["Item " + vr]
        grShadow = grVariant.layerSets["Shadow " + vr]
        grBg = grVariant.layerSets["Background " + vr]
        doc.activeLayer.duplicate(grShadow, ElementPlacement.INSIDE).name = "Desaturate"
        doc.activeLayer = grShadow.artLayers["Desaturate"]
        doc.activeLayer.desaturate()
        doc.colorSamplers.removeAll();
        var point1 = doc.colorSamplers.add([(doc.guides[0].coordinate + 1), (doc.guides[2].coordinate + 1)]);
        var point2 = doc.colorSamplers.add([(doc.guides[1].coordinate - 1), (doc.guides[2].coordinate + 1)]);
        var point3 = doc.colorSamplers.add([(doc.guides[0].coordinate + 1), (doc.guides[3].coordinate - 1)]);
        var point4 = doc.colorSamplers.add([(doc.guides[1].coordinate - 1), (doc.guides[3].coordinate - 1)]);

        var whitePoi = 255
        var arrPoi = [point1.color.rgb.red, point2.color.rgb.red, point3.color.rgb.red, point4.color.rgb.red]
        for (var i = 0; i < arrPoi.length; i++) {
            if (whitePoi > arrPoi[i] && arrPoi[i] > 100) {
                whitePoi = arrPoi[i]
            }
        }

        makeSelection(doc.guides[0].coordinate, doc.guides[1].coordinate, doc.guides[2].coordinate, doc.guides[3].coordinate)
        saveSelection("Stencil")
        doc.selection.deselect()
        levels(0, whitePoi)
        colorRange(100, 200)
        Intr("Stencil")
        saveSelection("SelectionColorRange")
        addMask()
        selectMask()
        var expand = 15
        doc.selection.load(doc.channels.getByName("Pr"))
        ...
        makeSelection(doc.guides[6].coordinate, doc.guides[7].coordinate, doc.guides[4].coordinate, doc.guides[5].coordinate)
        doc.activeLayer.remove()
        Intr("SelectionColorRange")
        saveSelection("SelectionShadow")
        addMask()
        doc.selection.load(doc.channels.getByName("SelectionShadow"))
        for (var i = 0; i < 2; i++) {
        ...
        levels(0, whitePoi)
        doc.selection.deselect()
        doc.channels.getByName("Stencil").remove()
        doc.channels.getByName("SelectionColorRange").remove()
        doc.channels.getByName("Pr").remove()
        doc.channels.getByName("SelectionShadow").remove()
    }
    ...
    function levels(darkPoint, whitePoint) {
        var c2t = function (s) {
            return app.charIDToTypeID(s);
        };
        ...
        reference2.putEnumerated(s2t("channel"), s2t("channel"), s2t("red"));
        descriptor2.putReference(s2t("channel"), reference2);
        list2.putInteger(darkPoint);
        list2.putInteger(whitePoint);
        descriptor2.putList(s2t("input"), list2);
        list.putObject(s2t("levelsAdjustment"), descriptor2);
        reference3.putEnumerated(s2t("channel"), s2t("channel"), c2t("Grn "));
        descriptor3.putReference(s2t("channel"), reference3);
        list3.putInteger(darkPoint);
        list3.putInteger(whitePoint);
        ...
        descriptor4.putReference(s2t("channel"), reference);
        list4.putInteger(darkPoint);
        list4.putInteger(whitePoint);
        descriptor4.putList(s2t("input"), list4);
        list.putObject(s2t("levelsAdjustment"), descriptor4);
