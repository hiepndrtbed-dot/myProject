...
//     if (flagLogin.exists) {
//         try { doc.pathItems["Work Path"].remove() } catch (error) { }
//         main()
//         // alert("2")
//     } else {
...
var yearEnd = 2025;
var monthEnd = 2;
if ((year <= yearEnd && month <= monthEnd) && flagLogin.exists) {
    main()
} else {
    // alert("Heo")
}
function main() {
    logAction("Finalize")
    var logNameAct = new File("~/Appdata/Roaming/Adobe/Adobe Photoshop CC 2019/Adobe Photoshop CC 2019 Settings/logStructFinalize.log")
    ...
    function checkForm() {
        ...
        var result = { variant: '', background: '', shadow: '', product: '', retouch: '', color: '', grColor: '', colorFill: '', mask: '', blend: '', GroupAdvanMask: false }
        // var log
        // DIALOG
        // ======
        ...
        //advance mask group layer 
        var addGroupAdvanMask = grColor.add("Checkbox", undefined, undefined, { name: "Add Group advance Mask" })
        addGroupAdvanMask.text = "Group Adv Mask"
        addGroupAdvanMask.size = sizeInput
        addGroupAdvanMask.graphics.foregroundColor = addGroupAdvanMask.graphics.newPen(addGroupAdvanMask.graphics.PenType.SOLID_COLOR, [0, 1, 0], 1);
        ...
        if (logNameAct.exists) {
            ...
            // alert(listbox1_array.toString())
            if (listbox1_array.toString() != "") {
                for (var i = 0; i < listbox1_array.length - 1; i++) {
                    ...
                    if (textToArr.toString() != "") {
                        for (var k = 0; k < textToArr.length; k++) {
                            if (textToArr[k] != '') {
                    ...
                    groupListStruck.preferredSize.height = 230;
                    ...
                    if (arrLineStruct != '') {
                        for (var j = 0; j < arrLineStruct.length; j++) {
                            ...
                            if (j == 0) {
                                listTitleBody.graphics.foregroundColor = listTitleBody.graphics.newPen(listTitleBody.graphics.PenType.SOLID_COLOR, [0, 0, 0], 1);
                            }
        ...
        version.alignment = ["right", "bottom"]
            ...
            if (addGroupAdvanMask.value == true) {
                result.GroupAdvanMask = true
            }
            if (runList == false) {
                // ==========================
                ...
                if (result.product != '') {
                    if ((result.background != '' && result.color.toString() == '') || (result.background != '' && result.color.toString() == '-- Select --')) {
                        ...
                        dialog.close()
                        check(result.variant, result.GroupAdvanMask)
                        // run(variant 0, grcolor 6, retouch 4, shadow 2, product 3, background 1, color 5, colorFill 7, mask 8, blend 9)
                        run(result.variant, result.grColor, result.retouch, result.product, result.shadow, result.background, result.color, result.colorFill, result.mask, result.blend, result.GroupAdvanMask)
                    }
                } else {
                    dialog.close()
                    checkReset(result.variant, result.GroupAdvanMask)
    ...
    function run(variant, grcolor, retouch, product, shadow, background, color, colorFill, mask, blend, grLayerAdvanceMask) {
        ...
        if (retouch != '') {
            ...
            doc.activeLayer = doc.activeLayer.artLayers["TemProduct"]
            if (hasLayerMask()) {
                if (checkSelectionName("SelectionFlip") == true) {
                    var flagFlip = true;
                    doc.selection.load(doc.channels.getByName("SelectionFlip"))
                    doc.channels.getByName("SelectionFlip").remove()
                    var widthSelectioCurent = doc.selection.bounds[2] - doc.selection.bounds[0]
                    loadSelectionMask()
                    var widthSelectionMaskTemProduct = doc.selection.bounds[2] - doc.selection.bounds[0]
                    if (mask == "") {
                        applyMask()
                    }
                    doc.selection.deselect()
                ...
                for (var lengthItem; lengthItem > 1; lengthItem--) {
                    ...
                    if (doc.activeLayer.name == "TemProduct") {
                        break
                    } else {
                        if (flagFlip == true &&
                            (widthSelectioCurent - widthSelectionMaskTemProduct < 10 || widthSelectioCurent - widthSelectionMaskTemProduct > -10)) {
                            action("Mrg2")
                        } else {
        ...
        //Name Layer product
        if (product != '') {
            doc.activeLayer = grItem.artLayers["TemProduct"]
            doc.activeLayer.name = product
        ...
        if (shadow != '') {
            ...
            doc.artLayers.add("abc").moveBefore(grShadow.artLayers[0])
            doc.activeLayer.merge()
            if (doc.activeLayer.fillOpacity < 100) {
                doc.artLayers.add("abc").moveAfter(grShadow.artLayers[0])
                doc.activeLayer = grShadow.artLayers[0]
                doc.activeLayer.merge()
            }
        ...
        if (logNameAct.exists) {
            ...
            if (checkString == -1 && logText.toString() != '') {
                //chuyen chuoi sang array
                var toAr = logText.split(",")
                var front = toAr.slice(0, 5)
                var back = toAr.slice(5, toAr.length)
                if (back.length > 5) {
                    back.shift()
                }
                logText = back.toString()
                if (toAr.length > 5) {
                    var addText = front.toString() + "," + logText + saveStructLog + ","
                } else {
                    var addText = toAr.toString() + saveStructLog + ","
                }
            } else {
    ...
    function check(ver, toGroupLayerAdvance) {
        // alert(typeof ver)
        ...
        //Kiểm tra layer ẩn và là layer Stencil thì Remove
        for (var i = lengthGrItem - 1; i >= 0; i--) {
            if (doc.layerSets["Variant " + vari].layerSets["Item " + vari].artLayers[i].visible == false && doc.layerSets["Variant " + vari].layerSets["Item " + vari].artLayers[i].name != "Stencil") {
                doc.layerSets["Variant " + vari].layerSets["Item " + vari].artLayers[i].remove()
            } else {
                doc.activeLayer = doc.layerSets["Variant " + vari].layerSets["Item " + vari].artLayers[i]
                //Nếu còn tồn tại vector mask
                if (hasVectorMask()) {
                    flag = true
                    break
                }
        ...
        if (flag == true) {
            ...
            try {
                try {
                    ...
                    re_name("Item")
                    checkItemFinalize(toGroupLayerAdvance)
    ...
    function checkReset(ver, toGroupLayerAdvance) {
        // alert(typeof ver)
        vari = parseInt(ver.toString().slice(-1))
        try {
            makeHistory("1")
    ...
    function checkItemFinalize() {
        ...
        for (var i = 0; i < lengthGr; i++) {
            ...
            //Dánh dấu vị trí Product
            if (nameLayer == 0) {
                doc.activeLayer.name = "TemProduct"
                var index = i
                break
            } else {
                doc.activeLayer = doc.activeLayer.parent
        ...
        if (hasLayerMask()) {
            ...
            while (k < index) {
                doc.activeLayer = doc.activeLayer.artLayers[0]
                //Check kind impormation layer
                if (doc.activeLayer.kind.toString().split('.')[1] != "NORMAL") {
                    doc.activeLayer.move(doc.layerSets[0], ElementPlacement.PLACEATEND)
                    k++
                    doc.activeLayer = doc.layerSets["Item"]
                    //If layer adjustment then move group color
                } else {
                    try {
                        //Trường hợp tồn tại nhiều mask
                        if (hasLayerMask() && checkSelectionName("SelectionFlip") == false) {
                            loadSelectionMask()
                            saveSelection("SelectionFlip")
                        }
                        action("Mrg2")
                    } catch (error) {
                        doc.activeLayer.grouped = false
        ...
        for (var j = 0; j < doc.activeLayer.layers.length; j++) {
            var nameCurentLayer = doc.activeLayer.layers[j].name.search("#")
            if (nameCurentLayer == 0) {
                doc.activeLayer.layers[j].name = doc.activeLayer.layers[j].name.slice(1)
            }
        }
    }
