...
(function () {
    ...
    inputTemplate.addEventListener("click", function () {
        var inputFiles = Folder.selectDialog("Select folder Templates!")
    ...
    saveButton.onClick = function () {
        ...
        if (!template.exists) {
            alert("Please select folder template!");
            return;
        }
    ...
    function processImages(template, images, output, formatIndex, quality) {
        ...
        //Processing
        var filesTemp = template.getFiles();
        var filesImages = images.getFiles();
        valueProcessing = 100 / filesTemp.length
        for (var i in filesTemp) {
            // try {
            docTemp = open(filesTemp[i]);
            makeHistory("1")
            //load folder images.
            for (var y in filesImages) {
                selectHistory("1")
                try {
                    try {
                        selectLayer("Design");
                    } catch (error) {
                        selectLayer("Image Safe");
                    }
                    // activeDocument.activeLayer = activeDocument.layerSets["Canvas"].artLayers["Design"];
                    editContentSubject()
                    ...
                    if (extension === 'jpg' || extension === 'jpeg' || extension === 'png') {
                        {
                            ...
                            saveImage(output, formatIndex, filesTemp[i].name.split('.')[0] + "_" + y, quality)
                        }
                    }
                } catch (error) {
            ...
            valueProcessing = valueProcessing + 100 / filesTemp.length;
            // } catch (error) {
            //     alert(error)
            // }
