...
(function () {
    ...
    var dialog = new Window("dialog", "Insert logo and number!");
    ...
    function processImages(xlsx, template, images, output, formatIndex, quality) {
        ...
        for (var i = 0; i < dataFiles.length; i++) {
            ...
            var numberI = dataFiles[i][0]
            var logoI = dataFiles[i][1]

            //with curent layer same: Black Gold
            doc.activeLayer = doc.artLayers["number"]
            doc.selection.selectAll()
            replaceContents(images + "/" + numberI);
            doc.artLayers['number'].visible = false;
            logoI == true ? doc.artLayers["logo"].visible = true : doc.artLayers["logo"].visible = false;
            //Processing
       
            progressbar1.value = valueProcessing;
            saveImage(output, formatIndex, numberI.split(".")[0], quality);
