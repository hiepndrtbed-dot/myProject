...
(function () {
    ...
    function resizeImage(inputFileTemlate, widthImages, heightImages, marginTopBottomImage, marginLeftRightImage, colorBackground) {
        ...
        var marginTopBody = MedHeight / 3.2
        var marginLeftRightBody = marginLeftRightImage + 60
        ...
        if (radioWidth > radioHeight) {
            ...
            resizeLayer(MedWidth - 2 * marginLeftRightBody, MedHeight - (marginTopBottomImage + marginTopBody))
            doc.activeLayer.translate(0, marginTopBody)

            //Fill background
            var colorRGB = hexToRgb('#' + colorBackground);
            makeSolidColor(colorRGB[0], colorRGB[1], colorRGB[2])
            doc.activeLayer.name = "ColorFill"
            moveDown()

            //Replace and transform auto Header
            replaceContents(inputFileTemlate)

           
            var marginTopHeader = marginTopBottomImage 
            resizeLayer(doc.width - 2 * marginLeftRightImage, marginTopBody - marginTopHeader)

            var widthHeader = doc.activeLayer.bounds[2] - doc.activeLayer.bounds[0]
            var marginHeaderLeftRight = (MedWidth - widthHeader) / 2

            //Transform theo chieu doc
            if (doc.activeLayer.bounds[1] > 0) {
                if (doc.activeLayer.bounds[1] > marginTopHeader) {
                    doc.activeLayer.translate(0, -(doc.activeLayer.bounds[1] - marginTopHeader))
                } else if (doc.activeLayer.bounds[1] < marginTopHeader) {
                    doc.activeLayer.translate(0, marginTopHeader - doc.activeLayer.bounds[1])
                }
            } else if (doc.activeLayer.bounds[1] < 0) {
                doc.activeLayer.translate(0, marginTopHeader + (doc.activeLayer.bounds[1] * -1))
            } else {
                doc.activeLayer.translate(0, marginTopHeader)
            }
            //Transform theo chieu ngang
            if (doc.activeLayer.bounds[0] > 0) {
                if (doc.activeLayer.bounds[0] > marginHeaderLeftRight) {
                    doc.activeLayer.translate(-(doc.activeLayer.bounds[0] - marginHeaderLeftRight), 0)
                } else if (doc.activeLayer.bounds[0] < marginHeaderLeftRight) {
                    doc.activeLayer.translate(marginHeaderLeftRight - doc.activeLayer.bounds[0], 0)
                }
            } else if (doc.activeLayer.bounds[0] < 0) {
                doc.activeLayer.translate(marginHeaderLeftRight + (doc.activeLayer.bounds[0] * -1), 0)
            } else {
                doc.activeLayer.translate(marginHeaderLeftRight, 0)
            }
...
function resizeLayer(width, height) {
    ...
    app.activeDocument.activeLayer.resize(resizePercent, resizePercent, AnchorPosition.TOPCENTER);
}
